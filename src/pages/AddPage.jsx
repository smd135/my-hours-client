...
import { createRoute } from '../redux/features/routesSlice';
...
const AddPage = () => {
	...
	const submitHandler = (e) => {
		e.preventDefault();
		try {
			dispatch(createRoute({ route_num, start_at, end_at, engine_type, engine_num, train_num, route_etc }));
		...
		<div>
			<form onSubmit={submitHandler} className="addForm w-full mx-auto px-1 py-10">
				<Space direction="vertical" className="w-full py-1.5">
					...
					<input type="number" placeholder="номер локомотива" style={{ width: '100%' }} name="engine_num" className="addInput w-full" value={engine_num} onChange={onChange} />

					<label className="text-sm text-white opacity-70">Номер поїзда:</label>
					<input type="number" placeholder="номер поїзда" style={{ width: '100%' }} name="train_num" className="addInput w-full" value={train_num} onChange={onChange} />
